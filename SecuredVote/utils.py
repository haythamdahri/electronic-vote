...
def decrypt_pending(pending):
    try:
        ...
        unused_candidate_id_ciphertext = co_file.readline().decode('unicode-escape').strip()[2:-1].encode('ISO-8859-1')

        # co_file (Clair text separated with \n)
        co_file = open(pending.de_file.path, "rb")
        unused_voter_id_ciphertext = co_file.readline().decode('unicode-escape').strip()[2:-1].encode('ISO-8859-1')
        ...
        # candidate_identifier_ciphertext_cipher = de_public_key.encrypt(
...
def decrypt_revision(revision):
    try:
        ...
        voter_id_from_voter = de_private_key.decrypt(
        ...
        voter_id_hashed_from_voter = make_hash(voter_id_from_voter)
        ...
        print(f"voter_id_hashed_from_voter: {voter_id_hashed_from_co}")

        # Get candidate id from co
        candidate_id_from_co = de_private_key.decrypt(
        ...
        print(voter_id_from_co_ciphertext)
        print(voter_id_from_voter_ciphertext)

        assert voter_candidate == co_candidate and voter_id_hashed_from_co == voter_id_hashed_from_voter, "Invalid vote"
